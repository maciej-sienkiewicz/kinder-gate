...
/**
 * Returns installed user-facing apps merged with monitored status.
 * Filters out system apps by default (parent can opt-in to see all).
 * Result is sorted: monitored first, then alphabetically.
 */
class GetInstalledAppsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(includeSystemApps: Boolean = false): List<InstalledApp> {
        ...
        val monitoredPackages = monitoredAppsRepository.getEnabledPackageNames()
        ...
        return installedPackages
            ...
            .map { appInfo ->
                InstalledApp(
                    ...
                    isMonitored = appInfo.packageName in monitoredPackages
                )
            }
            .sortedWith(compareByDescending<InstalledApp> { it.isMonitored }.thenBy { it.label })
    }
}
